...
table thead tr th{
	padding:10px 0;
	border-top: 2px solid #2ead18;
	border-bottom : 1px solid #8ed882;
}
table tbody tr td{
	padding:5px 0;
	border-bottom : 1px solid #9bb297;
}
        ...
        <table width="1000">
            ...
            <tbody>
                <c:forEach items="${list }" var="dto">
                    <tr>
                        <td class="td">${dto.hhhno }</td>
                        <td class="title"><a href="content-view?hNo=${dto.hhhno }">${dto.hhhtitle }</a>
                            <c:if test="${dto.hhhreplycount > 0 }">
                                <span id="recnt">(${dto.hhhreplycount })</span>
                            </c:if>
                            <c:if test="${dto.hhhlikecount >= 3 }">
                                <img src="resources/img/board/hot.gif">
                            </c:if>
                        </td>
                        <td class="td">${dto.hhhnickname }</td>
                        <td class="td">${dto.hhhregdate }</td>
                        <td class="td">${dto.hhhcount }</td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
        <ul id="ul_w">
        	<li><a id="btn_01" href="write-view">글쓰기</a></li>
		</ul>
        <form id="search" name="searchform" action="board">
            <select name="searchField">
                <option value="hhhtitle" selected>제목
                <option value="hhhcontent">내용
                <option value="both">제목+내용
                <option value="hhhnickname">글쓴이
            </select>
